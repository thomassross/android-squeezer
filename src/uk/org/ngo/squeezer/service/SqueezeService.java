...
public class SqueezeService extends Service {
    ...
    public void onCreate() {
        ...
        connectionState.setWifiLock(((WifiManager) getSystemService(Context.WIFI_SERVICE))
                .createWifiLock(
    ...
    private Map<String, SqueezerCmdHandler> initializeGlobalHandlers() {
        ...
        for (final SqueezerCLIImpl.ExtendedQueryFormatCmd cmd : cli.extQueryFormatCmds)
            if (!(cmd.playerSpecific || cmd.prefixed))
                handlers.put(cmd.cmd, new SqueezerCmdHandler() {
                    public void handle(List<String> tokens) {
                ...
                if ("delete".equals(tokens.get(1)))
                    ...
                    if (tokenMap.get("overwritten_playlist_id") != null) {
                        if (playlistMaintenanceCallback.get() != null) {
                            try {
                                playlistMaintenanceCallback.get().onCreateFailed(
                    ...
                    if (tokenMap.get("dry_run") != null) {
                        if (tokenMap.get("overwritten_playlist_id") != null) {
                            if (playlistMaintenanceCallback.get() != null) {
                                try {
                                    playlistMaintenanceCallback.get().onRenameFailed(
                                            getString(R.string.PLAYLIST_EXISTS_MESSAGE,
                            ...
                            cli.sendCommand("playlists rename playlist_id:"
                                    + tokenMap.get("playlist_id") + " newname:"
                                    + Util.encode(tokenMap.get("newname")));
                    }
    ...
    private Map<String, SqueezerCmdHandler> initializePlayerSpecificHandlers() {
        ...
        handlers.put("prefset", new SqueezerCmdHandler() {
            public void handle(List<String> tokens) {
                if (tokens.size() > 4 && tokens.get(2).equals("server")
                        && tokens.get(3).equals("volume")) {
    ...
    void onLineReceived(String serverLine) {
        ...
        // Player-specific commands follow. But we ignore all that aren't for
        // our active player.
    ...
    private void onServerStateChanged(SqueezerServerState oldServerState) {
        ...
        while (i > 0) {
            i--;
            try {
                albumListCallbacks.getBroadcastItem(i).onServerStateChanged(oldServerState,
                        serverState);
            } catch (RemoteException e) {
        ...
        while (i > 0) {
            i--;
            try {
                artistListCallbacks.getBroadcastItem(i).onServerStateChanged(oldServerState,
                        serverState);
            } catch (RemoteException e) {
                // The RemoteCallbackList will take care of removing
                // the dead object for us.
    ...
    void changeActivePlayer(SqueezerPlayer newPlayer) {
        ...
        if (oldPlayerId == null || !oldPlayerId.equals(playerId)) {
            if (oldPlayerId != null)
                ...
                // the docs say, multiple subscribes can be active and flood
                // us.)
        ...
        if (changed) {
            ...
            // NOTE: this involves a write and can block (sqlite lookup via
            // binder call), so
            // should be done off-thread, so we can process service requests &
            // send our callback
    ...
    private void updateOngoingNotification() {
        ...
        if (playing) {
            status.setLatestEventInfo(this,
                            getString(R.string.notification_playing_text, playerName), songName,
                    pIntent);
            status.flags |= Notification.FLAG_ONGOING_EVENT;
            status.icon = R.drawable.stat_notify_musicplayer;
        } else {
            status.setLatestEventInfo(this,
                            getString(R.string.notification_connected_text, playerName), "-",
                    pIntent);
            status.flags |= Notification.FLAG_ONGOING_EVENT;
    ...
    private final ISqueezeService.Stub squeezeService = new ISqueezeService.Stub() {
        ...
        public boolean togglePausePlay() throws RemoteException {
            ...
            if (playerState.isPlaying())
                // NOTE: we never send ambiguous "pause" toggle commands
                // (without the '1') because then we'd get confused when they
                // came back in to us, not being able to differentiate ours
                // coming back on the listen channel vs. those of those idiots
                // at the dinner party messing around.
                cli.sendPlayerCommand("pause 1");
            else
                // TODO: use 'pause 0 <fade_in_secs>' to fade-in if we knew it
                // was
        ...
        public boolean playlistControl(String cmd, String className, String itemId)
                throws RemoteException {
            if (!isConnected())
                return false;
            cli.sendPlayerCommand("playlistcontrol cmd:" + cmd + " " + itemKeys.get(className)
                    + ":" + itemId);
            return true;
        ...
        public boolean pluginPlaylistControl(SqueezerPlugin plugin, String cmd, String itemId)
                throws RemoteException {
            if (!isConnected())
                return false;
        ...
        public void registerPlaylistMaintenanceCallback(IServicePlaylistMaintenanceCallback callback)
                throws RemoteException {
        ...
        public void unregisterPlaylistMaintenanceCallback(
                IServicePlaylistMaintenanceCallback callback) throws RemoteException {
        ...
        public boolean playlistsMove(SqueezerPlaylist playlist, int index, int toindex)
                throws RemoteException {
            if (!isConnected())
                return false;
        ...
        public boolean playlistsRename(SqueezerPlaylist playlist, String newname)
                throws RemoteException {
            if (!isConnected())
                return false;
            cli.sendCommand("playlists rename playlist_id:" + playlist.getId()
                    + " dry_run:1 newname:" + Util.encode(newname));
            return true;
        }
        ...
        /*
         * Start an asynchronous fetch of the squeezeservers radio application
         * plugins
         */
        ...
        /*
         * Start an asynchronous fetch of the squeezeservers items of the given
         * type
         */
        public boolean pluginItems(int start, SqueezerPlugin plugin, SqueezerPluginItem parent,
                String search) throws RemoteException {
            if (!isConnected())
                return false;
            List<String> parameters = new ArrayList<String>();
            if (parent != null)
